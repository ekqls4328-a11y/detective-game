...
const LocationModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-neutral-950 flex flex-col animate-fadeIn overflow-hidden">
      ...
      {/* 중앙 배경 이미지 및 히트박스 영역 (flex-1로 남는 공간 모두 차지, overflow-hidden으로 넘침 방지) */}
      ...
      {/* 💡 하단 패널 및 컨트롤러 (shrink-0으로 고정 영역 확보, absolute 제거) */}
      <div className="shrink-0 w-full z-[80] bg-neutral-950 border-t border-neutral-800 p-4 pb-8 flex flex-col gap-4">
        
        {/* 조사 텍스트 창 */}
        <div className="bg-neutral-900 border border-neutral-700 p-4 rounded-2xl shadow-inner flex flex-col gap-3 justify-center min-h-[5rem]">
          {/* 💡 텍스트가 길어지면 내부 스크롤이 생기도록 max-h-[25vh]와 overflow-y-auto 추가 */}
          <p className="text-sm leading-relaxed text-white max-h-[25vh] overflow-y-auto pr-1">
            {discoveryText}
          </p>
          ...
          {focusedPoint && !inventory?.includes(focusedPoint.id) && (
            <button 
              ...
              className="mt-2 w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 shrink-0"
            >
          ...
          {focusedPoint && inventory?.includes(focusedPoint.id) && (
            <div className="mt-2 w-full py-2 text-center text-xs text-emerald-400 font-bold bg-emerald-950/30 rounded-lg border border-emerald-900/50 shrink-0">
              이미 수집된 단서입니다
            </div>
          )}
        </div>
        ...
        <button 
          ...
          className={`w-full py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-2 transition-all shadow-xl shrink-0 ${
